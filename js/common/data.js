...
const VersesData = [
    ...
    { "week": 2, "A": { "subject": "하나님과 매일 만나는 생활", "reference": "히브리서 4:16", "text": "그러므로 우리는 긍휼하심을 받고 때를 따라 돕는 은혜를 얻기 위하여 은혜의 보좌 앞에 담대히 나아갈 것이니라" }, "B": { "subject": "하나님과 매일 만나는 생활", "reference": "예레미야애가 3:22~23", "text": "여호와의 인자와 긍휼이 무궁하시므로 우리가 진멸되지 아니함이니이다 이것들이 아침마다 새로우니 주의 성실하심이 크시도소이다" } },
    ...
    { "week": 8, "A": { "subject": "하나님은 누구신가", "reference": "로마서 11:36상", "text": "이는 만물이 주에게서 나오고 주로 말미암고 주에게로 돌아감이라" }, "B": { "subject": "하나님은 누구신가", "reference": "예레미야 31:3하", "text": "내가 영원한 사랑으로 너를 사랑하기기에 인자함으로 너를 이끌었다 하였노라" } },
    ...
    { "week": 15, "A": { "subject": "거듭난 사람", "reference": "디도서 3:5", "text": "우리를 구원하시되 우리가 행한 바 의로운 행위로 말미암지 아니하고 오직 그의 긍휼하심을 따라 중생의 씻음과 성령의 새롭게 하심으로 하셨나니" }, "B": { "subject": "거듭난 사람", "reference": "데살로니가전서 1:3~4", "text": "너희의 믿음의 역사와 사랑의 수고와 우리 주 예수 그리스도에 대한 소망의 인내를 우리 하나님 아버지 앞에서 끊임없이 기억함이니 하나님의 사랑하심을 받은 형제들아 너희를 택하심을 아노라" } },
...
(() => {
    const seedPrayers = [
        "우리 가정이 믿음 안에서 하나 되고, 서로를 더 깊이 이해하며 사랑하게 하소서.",
        "직장 동료들에게 선한 영향력을 끼치는 사람이 되길 원하며, 업무 가운데 주의 지혜가 함께하길 기도합니다.",
        "연약한 육신을 주님의 권능으로 치유하여 주시고, 날마다 새 힘을 공급하여 주소서.",
        "자녀가 주님의 지혜와 명철로 자라나게 하시고, 하나님의 꿈을 꾸는 인생이 되게 인도하소서.",
        "이번 Grace Planner 훈련 기간 동안 주님의 세밀한 음성에 귀 기울이며 영적으로 성장하는 시간이 되게 하소서.",
        "경제적인 어려움과 불안함 가운데서도 주님만을 온전히 신뢰하는 담대한 믿음을 허락하소서.",
        "주변의 믿지 않는 가족과 친구들에게 그리스도의 향기를 전하고 복음의 통로로 쓰임받게 하소서.",
        "매일의 삶이 예배가 되게 하시고, 아침마다 주시는 말씀이 제 삶의 실제적인 이정표가 되게 하소서.",
        "주님의 뜻이 어디에 있는지 분별할 수 있는 영적 분별력을 주시고 순종할 마음을 부어주소서.",
        "교회와 공동체 안에서 낮은 자세로 이웃을 섬기며, 주님의 사랑을 실천하는 제자가 되게 하소서."
    ];

    const STORAGE_KEYS = {
        REGISTERED_USERS: 'gw_registered_users',
        PRAYER_JOURNAL_PREFIX: 'gw_prayer_journal_'
    };

    const users = window.Utils.getStorageItem(STORAGE_KEYS.REGISTERED_USERS, []);
    const believers = users.filter(u => u.role === 'believer' && u.isApproved);

    if (believers.length > 0) {
        believers.forEach(user => {
            const key = `${STORAGE_KEYS.PRAYER_JOURNAL_PREFIX}${user.id}`;
            const existing = window.Utils.getStorageItem(key, []);

            if (existing.length === 0) {
                const now = new Date();
                const dummyData = seedPrayers.map((desc, i) => {
                    const date = new Date(now);
                    date.setDate(now.getDate() - (i * 2)); // Subtract 2 days for each item
                    return {
                ...
                window.Utils.setStorageItem(key, dummyData);
                console.log(`Seeded 10 prayers for user: ${user.name}`);
            }
        });
    }
})();

window.VersesData = VersesData;
